...
URL=$(jq -r '.architectures.aarch64.url' "$MANIFEST")
EXPECTED_SHA256=$(jq -r '.architectures.aarch64.sha256' "$MANIFEST")
EXPECTED_SIZE=$(jq -r '.architectures.aarch64.size' "$MANIFEST")
VERSION=$(jq -r '.architectures.aarch64.version' "$MANIFEST")
